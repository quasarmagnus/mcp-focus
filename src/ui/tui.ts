...
type StateKey = "enabled" | "disabled" | "stub";
      ...
      visibleTools.length === 0
        ...
        : visibleTools
            .map((tool, i) => {
              ...
              return `  ${pointer} ${ICONS[k]}  ${display.padEnd(nameWidth)} ${k}`;
            })
            .join("\n");

    const hint =
      "  ↑↓ navigate  Space cycle state  ← / Esc back  type to filter  1 enable-all  2 disable-all  3 stub-all  Enter save  Ctrl+C cancel";
...
export async function runTuiAllServers(configManager: ConfigManager): Promise<void> {
  ...
  while (true) {
    ...
    const items: PickerItem[] = [
      { value: ALL, label: `\x1b[94mAll MCP Servers  (${totalTools} tool${totalTools !== 1 ? "s" : ""})\x1b[0m` },
      null,
      { header: "\x1b[94mConfigure Individual MCP:\x1b[0m" },
      ...serverNames.map((name) => {
        const count = Object.keys(config.servers[name]!.tools).length;
        return { value: name, label: `${name}  (${count} tool${count !== 1 ? "s" : ""})` };
      }),
      null,
